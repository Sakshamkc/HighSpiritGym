...
namespace HighSpiritApp.Services
{
    ...
    public class BoxingService : IBoxingService
    {
        ...
        {
            return await _boxingRepository.GetByCategoryAsync(category);
        }

        public async Task<IEnumerable<BoxingMember>> SearchByCategoryAsync(string? searchTerm, string category)
        {
            return await _boxingRepository.SearchByCategoryAsync(searchTerm, category);
        }

        public async Task<ImportResult> ImportFromExcelAsync(Stream fileStream, string category)
        {
            var result = new ImportResult { Success = true };
        ...
        public async Task<BoxingStats> GetStatsByCategoryAsync(string category)
        {
            var all = await _boxingRepository.GetByCategoryAsync(category);
            var members = all.ToList();
